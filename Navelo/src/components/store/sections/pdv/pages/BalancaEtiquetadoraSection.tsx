...
function BalancaEtiquetadoraConfigCard({
  enabled,
  setEnabled,
  retornoCodigo,
  setRetornoCodigo,
  ...
  retornoCodigo: "peso" | "valor"
  setRetornoCodigo: (v: "peso" | "valor") => void
  onNavigate: (subView: string) => void
}) {
...
function BalancaEtiquetadoraExportCard({
  modeloBalanca,
  setModeloBalanca,
  onExport,
}: {
  modeloBalanca: string
  setModeloBalanca: (v: string) => void
  onExport: () => void
}) {
  const s = UI_STRINGS.scales
  return (
    <Box bg="bg-white" border borderColor="border-border" radius="default" padding={5} w="full">
      <Stack gap={5} w="full">
        <Stack gap={1}>
          ...
          <Font variant="description" text={s.exportProductsDesc} color="muted" />
        </Stack>
        <Stack gap={1} w="full">
          <Font variant="sub-tiny-bold" text={s.scaleModelLabel} />
          <CustomSelect value={modeloBalanca} onChange={setModeloBalanca}>
        ...
        <Box w="full" display="flex" justify="end">
          <Button type="button" variant="outline" label={UI_STRINGS.common.export} icon={Download} onClick={onExport} />
        </Box>
      </Stack>
    </Box>
  )
...
export const BalancaEtiquetadoraSection: React.FC<BalancaEtiquetadoraSectionProps> = ({
  ...
  return (
    <Stack gap={5} w="full">
      <BalancaEtiquetadoraConfigCard
        enabled={enabled}
        setEnabled={setEnabled}
        retornoCodigo={retornoCodigo}
        setRetornoCodigo={setRetornoCodigo}
        onNavigate={onNavigate}
      />
      <BalancaEtiquetadoraExportCard
        modeloBalanca={modeloBalanca}
        setModeloBalanca={setModeloBalanca}
        onExport={() => setModalMsg("Exportando arquivo de carga de produtos...")}
      />
